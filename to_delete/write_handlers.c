...
/**
  ...
  *@ind: index at which the number starts on the bufer
  *@buffer: buffer
  *@flags: Flags
  *@width: Width
  *@prec: precision
		...
		int length, char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		buffer[ind] = padd = ' '; /*width dispalayed with padd */
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1 ; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ') /* Asign extra char left*/
		{
			if (extra_c)
				buffer[--ind] = extra_c;
	...
	int write_pointer(char buffer[], int ind, int length,
			int widt, int flags, char padd, char extra_c, int padd_start)
	{
		int i;
		if (width > length)
		{
			...
			if (flags & F_MINUS && padd == ' ')
			{
				...
				return (write(1, &buffer[ind], length) + write(1,
					...
					else if (!(flaga & F_MINUS) && padd == '0') /* extra char to
										       left of padd */
					{
					if (extra_c)
					buffer[--padd_start] = extra_c;
